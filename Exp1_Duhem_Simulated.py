...
def model(y, t):
    dydt = 0.4*np.abs(t*np.cos(t) + np.sin(t))*t*np.sin(t) - 0.5 * np.abs(t*np.cos(t) + np.sin(t))*y + 0.25*(t*np.cos(t) + np.sin(t))
    return dydt
...
c3 = 0.251
def test_model(y, t):
    dydt = c1*np.abs(t*np.cos(t) + np.sin(t))*t*np.sin(t) - c2 * np.abs(t*np.cos(t) + np.sin(t))*y + \
           c3*(t*np.cos(t) + np.sin(t))
    return dydt
